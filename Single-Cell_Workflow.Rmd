...
```
#Kneeplot Cell Identification
##Generating kneeplots in accordance with the Drop-Seq Analysis Cookbook guidelines
```{r}
# Mice = a vector of mouse identifiers
mice <- paste0("MB", c(4:7, 9:10, 12:15))
# Create iterable function to generate knee plot figures
kneeplot <- function(mouse, root, xlim){
  message('Working on Mouse: ', mouse)
  rh <- read.table(paste0(root,
                          mouse, 
                          "/out/XCTagHistogram.txt"))
  x <- cumsum(rh$V1)
  x <- x/max(x)
  kp <- plot(1:length(x), 
        x, 
        ...
        xlim=c(1, xlim),
        main=paste0(mouse, " Knee Plot"))
  return(kp)
}
# Create knee plots. The position of the "knee" will inform how many cells to include in the initial expression matrix
pdf('KneePlots.pdf')
lapply(mice, function(i) kneeplot(i, '/pine/scr/b/e/benb3/', 10000))
dev.off()
```
...
GT <- MergeSeurat(seurats[['MB4']], seurats[['MB5']])
for(mouse in mice[3:length(mice)]){
  GT <- MergeSeurat(GT, seurats[[mouse]])
}
# Addint metadata (drug treatment, sex & run date) to the merged object, using mouse ID as a key
...
GT <- SetAllIdent(object = GT, id = "Replicate")
# Gather percent mitochondrial transcripts for each cell (a marker for dying cells) within the Seurat object
...
# Outputting initial "unselected" cell metrics.
# Loosely, the cells in the object are all cells above the knee-plot "knee" by cumulative reads. We want to gather these raw UMI and gene metadata
df <- data.frame()
GetCellData <- function(object, ident.use){
  object <- SetAllIdent(object, ident.use)
  for(m in levels(object@ident)){
    ms <- SubsetData(object, ident.use = m)
    df["nCells", m] <- ncol(ms@data)
    df[c('min.UMI', 'max.UMI'), m] <- range(ms@meta.data$nUMI)
    df[c('min.Gene', 'max.Gene'), m]<- range(ms@meta.data$nGene)
    df[c('min.pct.mito', 'max.pct.mito'), m]<- range(ms@meta.data$percent.mito)
  }
  return(df)
}
df <- GetCellData(GT, "Replicate")
write.xlsx(df, "cell-data.xlsx", sheetName = "raw.data", append =T)
```
#Apply Data Quality Filters
###(Many are data-set specific, owing to differing depths of sequence by library batch)
```{r}
# Plot metadata features and filter cells on raw thresholds to center cells around the "belly" of the violin plot
VlnPlot(object = GT, 
        features.plot = c("nGene", "nUMI", "percent.mito"), nCol = 3, group.by = "Date")
# Empty Seurats list in preparation for holding filtered objects
seurats <- list()

seurats[['MB4']] <- FilterCells(object = GT, 
                  ...
                  cells.use = WhichCells(GT, subset.name = "Replicate",
                                         accept.value = "MB4"))
seurats[['MB5']] <- FilterCells(object = GT, 
                  subset.names = c("nGene", "nUMI", "percent.mito"), 
                  low.thresholds = c(500, 200, -Inf), 
                  high.thresholds = c(2000, 4000, 5),
                  cells.use = WhichCells(GT, subset.name = "Replicate",
                                         accept.value = "MB5"))
seurats[['MB6']] <- FilterCells(object = GT, 
                  subset.names = c("nGene", "nUMI", "percent.mito"), 
                  low.thresholds = c(500, 200, -Inf), 
                  high.thresholds = c(3000, 5000, 5),
                  cells.use = WhichCells(GT, subset.name = "Replicate",
                                         accept.value = "MB6"))
seurats[['MB7']] <- FilterCells(object = GT, 
                  subset.names = c("nGene", "nUMI", "percent.mito"), 
                  low.thresholds = c(500, 200, -Inf), 
                  high.thresholds = c(2200, 4000, 5),
                  cells.use = WhichCells(GT, subset.name = "Replicate",
                                         accept.value = "MB7"))
seurats[['MB9']] <- FilterCells(object = GT, 
                  subset.names = c("nGene", "nUMI", "percent.mito"), 
                  low.thresholds = c(500, 200, -Inf), 
...
# Visualizing the distributions once more, following selection
VlnPlot(object = GT,group.by = "Date", features.plot = c("nGene", "nUMI", "percent.mito"), nCol = 3)

# Outputting the "filtered" cell metrics.
# Loosely, the cells in the object are all cells above the knee-plot "knee", with visual outliers of the violin plots removed to center the "belly" of the violin plot more appropriately
df <- data.frame()
df <- GetCellData(GT, "Replicate")
write.xlsx(df, "cell-data.xlsx", sheetName = "filter.data", append =T)

```
#Investigate Large Data Set by tSNE & Clusters

```{r}
# Data scaling is better done with the maximum number of cells - we include all samples for scaling regardless of treatment status to achieve better results and to ensure both treatmen classes are scaled identically.
# Cells were filtered above, now we filter genes. First we save the original, unfiltered matrix to the "misc" slot of the Seurat object
GT@misc <- GT@raw.data
# Filter genes to only those expressed in more than 30 cells to prevent extreme low-abundance genes (found in less than 0.1% of the sample) from influencing data scaling
GT@raw.data  <- GT@raw.data[rowSums(GT@raw.data != 0) >= 30, ]
# Follow Seurat Highly Variable Genes selection
GT <- NormalizeData(GT)
GT <- FindVariableGenes(GT, x.low.cutoff = 0.0125, 
                        x.high.cutoff = 3, 
                        y.cutoff = 0.5, 
                        y.high.cutoff = Inf, 
                        num.bin = 20, do.plot = F)
# Apply linear scaling & centering to data
GT <- ScaleData(GT, vars.to.regress = 'nUMI')
```
###Seurat Workflow
```{r}
# Predict the number of significant PCs (Z-score percent of total variance > 2)
GT.pca <- TestPCA(GT, do.return = T)
# Compute PCs
GT <- RunPCA(GT, pcs.compute = 60, seed.use = 42, weight.by.var = T, rev.pca = F, use.imputed = F)
# Find cluster populations & tSNE coordinates for visualization using the predicted number of PCs
...
# Find differentially expressed gene markers for each cluster and write them to an excel file for manual analysis
...
df <- data.frame()
df <- GetCellData(GT, "Replicate")
write.xlsx(df, "cell-data.xlsx", sheetName = "selected.data", append =T)
```
...
min.genes <- 300
min.cells <- 0
# Re-run the prep.seurat function we used when importing the tumor expression matrices
seurats <- lapply(mice[1:5], function(x) prep.Seurat(x))
names(seurats) <- mice[1:5]
# Iteratively merge sample-specific seurats into a single object
WT <- MergeSeurat(seurats[[mice[1]]], seurats[[mice[2]]])
for(mouse in mice[3:5]){
  WT <- MergeSeurat(WT, seurats[[mouse]])
}
# Add Mitochondrial percentage as metadata. Same as with tumor samples.
...
# Stash un-trimmed data prior to cell and gene selection
WT@misc <- WT@raw.data
# Get Cell metrics for threshold reporting.
df <- data.frame()

df <- GetCellData(WT, "Replicate")
write.xlsx(df, "wt-cell-data.xlsx", sheetName = "raw.data", append =T)
```
###Initial Filtering
```{r}
# Check violin plot of pre-filtered cells
VlnPlot(WT, c("nUMI", "nGene", "percent.mito"), group.by = "Replicate")
# Subset off initial outliers. The WT data set is sequenced to a higher depth and is more consistent sample-to-sample than the GT data set was. We will employ a *slightly* different but more appropriate filtering method. We first remove the upper and lower end of what we would believe to be a cell. Then we establish upper thresholds 4 (or 5 in the case of mitos) standard deviations above the median.
WT <- SubsetData(WT, 
                 subset.name = 'nUMI', 
                 accept.high = 30000,
                 accept.low = 350)
...
max.mito <- 5*sd(WT@meta.data$percent.mito) + median(WT@meta.data$percent.mito)
# Subsetting on our upper thresholds. 500 genes & 600 UMIs is about the lower limit of what a cell must contain to be informative to us.
WT <- SubsetData(WT, 
                 subset.name = 'nGene', 
...
df <- GetCellData(WT, "Replicate")
write.xlsx(df, "wt-cell-data.xlsx", sheetName = "filter.data", append =T)
...
df <- GetCellData(WT, "Replicate")
write.xlsx(df, "wt-cell-data.xlsx", sheetName = "subset.data", append =T)
